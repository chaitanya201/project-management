...
export default function MidSemMarks() {
  ...
  const addMidSemMarks = async (e) => {
    ...
    try {
      const response = await axios.patch(
        `https://student-project-management.herokuapp.com/teacher/add-midsem-marks?teacherId=${
          user ? user._id : "12"
        }`,
        {
          problemStatement,
  ...
  const addEndSemMarks = async (e) => {
    ...
    try {
      const response = await axios.patch(
        `https://student-project-management.herokuapp.com/teacher/add-endsem-marks?teacherId=${
          user ? user._id : "12"
        }`,
        {
          projectRealization,
