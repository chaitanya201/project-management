  ...
  const addRemark = async (e) => {
    ...
    try {
      const response = await axios.patch(
        `https://student-project-management.herokuapp.com/projects/addRemark?teacherId=${
          user ? user._id : "12"
        }`,
  ...
  const getAllPendingProjects = async () => {
    try {
      ...
      const response = await axios.get(
        "https://student-project-management.herokuapp.com/teacher/get-all-projects?teacherId=" +
          cookies.user._id +
          "&year=" +
          year.value +
          "&subject=" +
